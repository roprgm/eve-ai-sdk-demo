...
import { none, routeAuth } from "eve/channels/auth";
...
export default defineChannel({
  ...
  routes: [
    POST("/eve/web", async (request: Request, { send }: RouteHandlerArgs) => {
      ...
      const session = await send(chat.message, {
        auth: authContext,
    ...
    GET(
      "/eve/web/:id",
      async (request: Request, { getSession, params, resolveActiveSession }: RouteHandlerArgs) => {
        const authContext = await routeAuth(request, auth);
        if (authContext instanceof Response) {
          return authContext;
        }
